...
export async function webhookHandler(req:Request,res:Response) {
    ...
    try {
        logger.info('Constructing Web Hook Event');
        event = stripe.webhooks.constructEvent(req.body as Buffer,sig as string,process.env.STRIPE_WEBHOOK_SECRET as string);

    } catch (err: any) {
        logger.info('Verification of Webhook Signature failed');
        return res.status(400).send(`Webhook Error: ${err?.message}`)
    }

    try {
        logger.info('Billing Webhook Service Called');
        const result = await service.handleEvent(event,logger);
        if(!result.ok) {
            ...
            logger.error({error: result.message},'Webhook processed with error:');
